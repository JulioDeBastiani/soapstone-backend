...
namespace Soapstone.Domain.Interfaces
{
    public static class ReposiotryExtensions
    {
        ...
        public static Task<IEnumerable<TEntity>> GetPageDescendingAsync<TEntity>(this IRepository<TEntity> repository, Func<TEntity, object> orderBy, int skip = PaginationDefaults.DefaultSkip, int take = PaginationDefaults.DefaultTake) where TEntity : Entity
            => repository.GetPageDescendingAsync(null, orderBy, null, skip, take);

        public static Task<IEnumerable<TEntity>> GetPageDescendingAsync<TEntity>(this IRepository<TEntity> repository, Func<TEntity, bool> predicate, Func<TEntity, object> orderBy, int skip = PaginationDefaults.DefaultSkip, int take = PaginationDefaults.DefaultTake) where TEntity : Entity
            => repository.GetPageDescendingAsync(predicate, orderBy, null, skip, take);
